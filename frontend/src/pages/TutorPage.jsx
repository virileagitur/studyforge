        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div
                className={`max-w-[80%] px-4 py-3 rounded-xl leading-relaxed ${msg.role === 'user' ? 'bg-[var(--color-accent)] text-white' : 'bg-[var(--color-background)] text-[var(--color-text-primary)]'}`}
