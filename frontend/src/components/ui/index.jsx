...
export function Card({ children, className = '', onClick, hover = false }) {
  return (
    <div
      ...
      style={{ boxShadow: 'var(--shadow-card)', background: 'var(--color-card)' }}
      onClick={onClick}
    >
      {children}
    </div>
...
export function Button({ children, onClick, type = 'button', variant = 'primary', disabled = false, size = 'md', className = '', icon: Icon }) {
  const base = 'inline-flex items-center justify-center gap-2 font-semibold rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8DA9A0] disabled:opacity-50 disabled:cursor-not-allowed';
  const sizes = { sm: 'px-3 py-2 text-sm min-h-[36px]', md: 'px-4 py-2.5 text-sm min-h-[44px]', lg: 'px-6 py-3 text-base min-h-[48px]' };
  const variants = {
    primary: 'bg-[#8DA9A0] text-white hover:bg-[#7A958E]',
    secondary: 'bg-[#D4A857] text-white hover:bg-[#C49A47]',
    ghost: 'bg-transparent text-[#4A4A4A] hover:bg-[#EEF4F2] hover:text-[#8DA9A0]',
    danger: 'bg-red-500 text-white hover:bg-red-600',
    outline: 'border border-[#8DA9A0] text-[#5F8B8B] hover:bg-[#EEF4F2] bg-transparent',
  };
  return (
...
export function Input({ label, error, className = '', ...props }) {
  return (
    <div className={`space-y-1 ${className}`}>
      ...
      <input
        className="w-full px-3 py-2.5 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#8DA9A0] min-h-[44px] border border-gray-200"
        style={{ boxShadow: 'var(--shadow-sm)', color: 'var(--color-text)' }}
        {...props}
      />
...
export function Select({ label, error, children, className = '', ...props }) {
  return (
    <div className={`space-y-1 ${className}`}>
      {label && <label className="block text-sm font-medium text-gray-700">{label}</label>}
      <select
        className="w-full px-3 py-2.5 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#8DA9A0] min-h-[44px] border border-gray-200 appearance-none"
        style={{ boxShadow: 'var(--shadow-sm)', color: 'var(--color-text)' }}
        {...props}
      >
        {children}
...
export function Textarea({ label, error, className = '', rows = 4, ...props }) {
  return (
    <div className={`space-y-1 ${className}`}>
      ...
      <textarea
        rows={rows}
        className="w-full px-3 py-2.5 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#8DA9A0] border border-gray-200 resize-vertical"
        style={{ boxShadow: 'var(--shadow-sm)', color: 'var(--color-text)' }}
        {...props}
      />
      {error && <p className="text-red-500 text-xs">{error}</p>}
...
export function Spinner({ size = 'md' }) {
  ...
  return (
    <div className={`${sizes[size]} border-4 border-[#EEF4F2] border-t-[#8DA9A0] rounded-full animate-spin`} />
  );
}
